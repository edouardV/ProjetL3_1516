...
public class Arene extends UnicastRemoteObject implements IAreneIHM, Runnable {
	...
	public boolean AeraOfEffect(int refRMI, HashMap<Integer, Point> refPersoAdv) throws RemoteException {
		...
		new AeraOfEffect(this, client, refPersoAdv).interagit();
					personnages.get(refRMI).executeAction();
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION) {
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					...
					new Bruler(this,client,clientAdv).interagit();
					personnages.get(refRMI).executeAction();
	...
	public boolean TireFleche(int refRMI, int refRMIAdv) throws RemoteException {
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			// on teste la distance entre les personnages
			if (distance <= Constantes.DISTANCE_MAX_INTERACTION) {
				Personnage pers = (Personnage) elementFromRef(refRMI);
				Personnage persAdv = (Personnage) elementFromRef(refRMIAdv);
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					console.log(Level.INFO, Constantes.nomClasse(this), 
							"Je tire une fleche " + nomRaccourciClient(refRMIAdv));
					consoleAdv.log(Level.INFO, Constantes.nomClasse(this), 
							"Je me suis fait tir√© dessus par " + nomRaccourciClient(refRMI));
					
					logger.info(Constantes.nomClasse(this), nomRaccourciClient(refRMI) + 
							" tir une fleche sur " + nomRaccourciClient(consoleAdv.getRefRMI()));
			
					new TirerFleche(this, client, clientAdv).interagit();
					personnages.get(refRMI).executeAction();
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION) {
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					...
					new CoupCritique(this, client, clientAdv).interagit();
					personnages.get(refRMI).executeAction();
	...
	public boolean Soigner(int refRMI, int refRMI2) throws RemoteException {
		...
		VuePersonnage client = personnages.get(refRMI);
		VuePersonnage client2 = personnages.get(refRMI2);
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			// sinon, on tente de jouer l'interaction
			IConsole console = consoleFromRef(refRMI);
			IConsole console2 = consoleFromRef(refRMI2);
			
			int distance = Calculs.distanceChebyshev(personnages.get(refRMI).getPosition(), 
					personnages.get(refRMI2).getPosition());

			// on teste la distance entre les personnages
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION) {
				Personnage pers = (Personnage) elementFromRef(refRMI);
				Personnage pers2 = (Personnage) elementFromRef(refRMI2);
				
				// on teste que les deux personnages soient en vie
				if (pers.estVivant() && pers2.estVivant()) {
					console.log(Level.INFO, Constantes.nomClasse(this), 
							"Je soigne " + nomRaccourciClient(refRMI2));
					console2.log(Level.INFO, Constantes.nomClasse(this), 
							"Je suis soign√© par " + nomRaccourciClient(refRMI));
					
					logger.info(Constantes.nomClasse(this), nomRaccourciClient(refRMI) + 
							" soigne " + nomRaccourciClient(console2.getRefRMI()));
			
					new Soigner(this, client, client2).interagit();
					personnages.get(refRMI).executeAction();
				} //if
			} //if
	...
	public boolean deplace(int refRMI, Point objectif) throws RemoteException {
		...
		if (client.isActionExecutee()) {
			...
			if(vitesse < 1){
				...
				IConsole console = consoleFromRef(refRMI);
				console.log(Level.INFO, Constantes.nomClasse(this), 
						"Je suis gelÈ " + nomRaccourciClient(refRMI));
				vitesse++;
