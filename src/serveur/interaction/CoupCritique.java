...
public class CoupCritique extends Interaction<VuePersonnage>  {
	...
	public void interagit() {
		try {
			...
			// degats
			if (perteVie > 0) {
				int chance_critique = Calculs.nombreAleatoire(0, 100);
				if (chance_critique <= 50){
					arene.incrementeCaractElement(defenseur, Caracteristique.VIE, -perteVie * 2);
					logs(Level.INFO, Constantes.nomRaccourciClient(attaquant) + " colle unCoupCritique ("
							+ perteVie * 2 + " points de degats) a " + Constantes.nomRaccourciClient(defenseur));
				}
				else { 
